...
- **`_Layout.cshtml` `<head>` to'liq meta to'plami:** har sahifa `ViewData` orqali o'z `Description`/`Canonical`/`OgImage`/`OgType`/`Robots`/`JsonLd` qiymatini bera oladi; berilmasa aqlli standartlar. Qo'shildi: `meta description`, `meta robots` + `googlebot` (maxfiy controllerlar uchun avto `noindex`), `link canonical`, `theme-color`, `keywords`, **Open Graph** (type/site_name/locale uz_UZ/title/description/url/image 1200×630), **Twitter Card** (summary_large_image), shartli **JSON-LD** `<script>`.
- **Maxfiy sahifalar avto-noindex:** `_Layout` da `seoNoindexControllers` ro'yxati (Feed, Chat, Profile, ReadingGoals, Quotes, Onboarding, Notifications, Stories, Books, Auth, App) → `noindex, follow`. Ommaviy (Home, Posts, **PublicProfile**) → `index, follow, max-image-preview:large`.
- **`wwwroot/robots.txt`** (statik): ommaviyni Allow, maxfiy yo'llarni Disallow (/feed,/chat,/profile,/reading-books,/quotes,/onboarding,/notifications,/stories,/auth,/hangfire,/books,/app), `Sitemap: https://kitobdagimen.uz/sitemap.xml`.
- **Dinamik `/sitemap.xml`:** `SeoController` (`[AllowAnonymous]`, `/sitemap.xml`, 1soat cache) + Application query `GetSitemapQuery`/`SitemapDto`/`GetSitemapQueryHandler` (`Features/Seo/...`). Statik sahifalar (/ , /qollanma) + barcha postlar (`/post/{username}/{slug}`, lastmod=CreatedAt) + ommaviy profillar (`/u/{username}`, username'i bor va ≥1 postli). `Uri.EscapeDataString` bilan segmentlar xavfsiz, baseUrl request'dan (prod/lokal to'g'ri).
- **JSON-LD strukturali ma'lumot** (`Web/Seo/SeoJsonLd.cs`, System.Text.Json bilan to'g'ri escape): landing = **WebSite + Organization** (@graph); post detali = **BlogPosting** (headline, author Person + /u havola, datePublished ISO, `about` = Book + muallif); ommaviy profil = **ProfilePage** (mainEntity Person + avatar + bio).
- **Sahifaga xos SEO:** landing (Index.cshtml) — boyitilgan description + kanonik `/` (barcha landing aliaslari birlashadi) + WebSite JSON-LD; Guide (`/qollanma`) — description; post detali — title "{Kitob} — {Muallif} taqrizi", description = taqriz matnidan HTMLsiz 160-belgili parcha (bo'sh bo'lsa fallback), og:image = post rasmi/muqova (absolyut, bo'lmasa standart og-image), article + BlogPosting; public profil (`/u/...`) — description (bio yoki "{ism} — {N} taqriz, {M} obunachi"), og:type profile, ProfilePage JSON-LD (faqat PublicProfile controllerda — /profile da emas).
- **OG rasm** (`wwwroot/img/og-image.png`, 1200×630): Pillow (Python) bilan generatsiya — brend krem foni + boyo'g'li emblemi (logo proporsiyasida) + "kitobdagimen.uz" wordmark + "O'zbek kitobxonlari uchun ijtimoiy platforma" tagline. Generator: `/tmp/gen_og.py` (DejaVu shriftlari). Telegram/FB/Twitterda ulashilganda chiroyli kartochka.
- **TEKSHIRILDI (jonli, Release binar, vaqtinchalik 5261 — `launchSettings` ASPNETCORE_URLS ni bekor qilgani uchun 5299 emas 5261'ga tushdi; foydalanuvchining watch'i o'sha payt ishlamayotgan edi; tekshiruvdan keyin jarayon TO'XTATILDI, 5261 BO'SHATILDI):** `/robots.txt` 200 to'g'ri; `/sitemap.xml` 200 `application/xml`, haqiqiy 4+ post + profil URL; landing — description/robots index/canonical/OG/WebSite+Organization JSON-LD bor; post detali — title/excerpt-description/article/BlogPosting (author+book+datePublished); rasmli postda og:image=haqiqiy muqova, rasmsizda standart og-image; `/u/...` — index/ProfilePage/og profile; `/profile` (maxfiy) anonim uchun 401 (indekslanmaydi); og-image.png 200 (27KB). Build **0 warning / 0 error**. DB faqat o'qildi (SELECT). **Foydalanuvchi yangi SEO'ni dev'da ko'rishi uchun dotnet watch ni bir marta to'liq qayta ishga tushirsin (yangi controller + Application query bor — rebuild kerak).**
- **FOYDALANUVCHI QILADIGAN QADAMLAR (kod emas, reyting uchun zarur):** (1) **Google Search Console**da kitobdagimen.uz domenini tasdiqlash + `https://kitobdagimen.uz/sitemap.xml` topshirish; (2) Bing Webmaster Tools; (3) ijtimoiy tarmoq/Google profillarida kitobdagimen.uz havolasi (backlink); (4) muntazam YANGI KONTENT (postlar/taqrizlar); (5) sahifa tezligi allaqachon yaxshi (statik CSS/JS, WebP). #1 ga chiqish raqobat + vaqt masalasi; texnik tomondan endi to'liq tayyor.
